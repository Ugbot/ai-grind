...
different agents/machines merge at character level — nobody's improvement is
lost to a last-writer-wins clobber.

## Driving it — the `skill_live` tool
...
- **Prefer `append` and `patch` over full rewrites.** Two concurrent full
  rewrites BOTH survive a CRDT merge — you get the document twice. Surgical
  edits merge cleanly.
...
- Skills shared across your machines that must stay identical without a git
  round-trip.
- NOT for the curated library under `skills/` — those are reviewed, committed
