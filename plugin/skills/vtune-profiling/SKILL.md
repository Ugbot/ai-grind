...
summary + `run_id`, queryable Polars frame, flame graph — and keeps the result
...
- **Software sampling** (default `hotspots`) works unprivileged, in VMs and
  ...
  admin/root; in most VMs they're unavailable — the cookbook's "Profiling
...
memory: `loads`, `stores`, `llc_miss_count`…) — `devtools_query(run_id,
columns=["schema"])` lists them.

### extra_args the wrapper understands

- `--pid N` — attach to a running process instead of launching `binary`
  (add `--duration 30` to bound the collection in seconds).
- `--result-dir D` — collect into / reuse a named result dir.
- `--report-only` — skip collection, re-decode an existing result dir.
...
`thread`. The result dir is self-contained — copy it off a server and open it
locally in the GUI.
