...
- **Exclusive (Exc%) / "self"** — samples where this function was the **leaf**
  (the CPU was *in* it). This is where cycles actually burn.
- **Inclusive (Inc%) / "total"** — samples where this function was **anywhere on
...
3. **Follow a wide column down** from a high-Inc% dispatcher to find *where* its
   ...
   the cost spreads.
4. **Plateaus of `memset`/`memcpy`/alloc/GC** mean copying/allocation cost — look
