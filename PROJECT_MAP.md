...
| `src/devtools_mcp/tracker/` | Progress tracker domain layer: `schema.py` (versioned migrations), `db.py` (WAL SQLite at `~/.devtools-mcp/tracker.db`, env `DEVTOOLS_MCP_TRACKER_DB`), `tasks.py`/`criteria.py`/`tags.py`/`commits.py`/`deps.py` (dependency edges + execution-plan resolver)/`issues.py`, `frames.py` (Polars views), `crdt.py`+`sync.py` (local-first replication: HLC, op-capture triggers, LWW merge, HTTP peer sync via the dashboard's `/api/crdt/`), `providers/` (GitHub REST via `GITHUB_TOKEN`, GitLab stub). Tools in `tools/tracker_tools.py` (10 `tracker_*` tools); tracker card views in `viz/` (`/tracker` board + detail pages); skills in `skills/authored/skills/tracker/` |
