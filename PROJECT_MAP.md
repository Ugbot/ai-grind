...
| `.mcp.json` | MCP server definition (`devtools-mcp`, stdio) — uses `${CLAUDE_PLUGIN_ROOT}` so it works as a bundled plugin. HTTP/SSE: `devtools-mcp --transport http\|sse --port N` (or `DEVTOOLS_MCP_TRANSPORT`) |
