...
1. Make sure the dashboard is running, then call the `devtools_dashboard` tool
...
- `station_sync` syncs all enabled domains; `domain='tasks'` for one;
...
- `station_session` drives live coordination: `start`/`update` a work session,
  `handoff` work to other members, `inbox`/`accept`/`decline` handoffs,
  `context` for the platform's onboarding packet.
- `station_link action='status'` reports auth state, links, per-rule watermarks
