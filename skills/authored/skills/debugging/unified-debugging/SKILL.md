...
1. **`debug_start(program=...)`** — launch. Adapter is sniffed from the file
...
2. **`debug(session_id, action=...)`** — the verbs:
   ...
   - Escape hatch: `command` (raw_command=...) — the debugger's native REPL
     (lldb commands, Python expressions, ...).
3. **`debug_inspect(session_id, what=...)`** — deeper looks, each stored as a
...
4. **`debug_stop(session_id)`** — tear down. Multi-stop sessions emit a
...
stop). The session stays live wherever the plan halts — continue
...
- **Kotlin (kotlin-debug-adapter)**: launch needs extra={"main_class": ...}
  ...
  are unsupported — prefer JDWP attach (attach_port=5005 with
  -agentlib:jdwp=...,address=*:5005).
- **Java (jdt.ls + java-debug)**: first import of a project takes 30–60s
  (the jdt.ls process is cached per project after that). Kotlin-only Gradle
  projects resolve poorly — use the kotlin adapter there.
- **ABAP (ADT REST, attach-only)**: debugging a SAP system needs
  ...
  listener per user per system — a live Eclipse ADT session for the same
  user conflicts.
