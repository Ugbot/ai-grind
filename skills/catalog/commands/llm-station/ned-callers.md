...
```bash
timeout 30 llm-station --run call_graph "function=$ARGUMENTS" "direction=callees" "depth=2" --workspace /Users/bengamble/llm-station 2>&1
```

2. What calls this function (callers):
```bash
timeout 30 llm-station --run call_graph "function=$ARGUMENTS" "direction=callers" "depth=2" --workspace /Users/bengamble/llm-station 2>&1
```

3. Immediate callers/callees with file locations:
```bash
timeout 30 llm-station --run call_hierarchy "function=$ARGUMENTS" "direction=both" --workspace /Users/bengamble/llm-station 2>&1
```
