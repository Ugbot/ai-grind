...
async def detect() -> list[InstalledTool]:
    ...
    return [InstalledTool(suite="pnpm", name=t, path=info.get("path") or "pnpm",
                          version=version, available=available) for t in _TOOLS]


async def run(
    tool: str = "deps", binary: str = "", args: list[str] | None = None,
    extra_args: list[str] | None = None, timeout: int = 1800, **kwargs: Any,
...
_DF_BUILDERS = {
    "deps": deps_df, "sync": deps_df, "outdated": deps_df, "build": deps_df, "test": deps_df,
    "audit": vulns_df, "tasks": available_tasks_df, "_default": deps_df,
}


def _register() -> None:
    register_backend(BackendSpec(
        suite="pnpm", tools=_TOOLS, detect=detect, run=run,
        df_builders=_DF_BUILDERS, format_summary=format_summary,
    ))
