...
async def detect() -> list[InstalledTool]:
    ...
    return [InstalledTool(suite="yarn", name=t, path=info.get("path") or "yarn",
...
_DF_BUILDERS = {
    "deps": deps_df, "sync": deps_df, "build": deps_df, "test": deps_df,
...
def _register() -> None:
    register_backend(BackendSpec(
        suite="yarn", tools=_TOOLS, detect=detect, run=run,
