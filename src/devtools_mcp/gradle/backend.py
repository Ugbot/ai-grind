...
async def detect() -> list[InstalledTool]:
    """Gradle via global gradle or a project gradlew wrapper (wrapper projects work)."""
...
async def run(
    ...
    **kwargs: Any,
    ...
    """Run a Gradle task via the runner. `binary` = project directory."""
    return await run_gradle(tool=tool, binary=binary, args=args, extra_args=extra_args, timeout=timeout, **kwargs)


def format_summary(result: RunBase) -> str:
    if isinstance(result, BuildResult):
        return format_build_summary(result)
    return f"Unknown gradle result: {type(result)}"
